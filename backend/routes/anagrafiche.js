...
router.get('/settori', verifyToken, async (req, res) => {
    const [rows] = await pool.query('SELECT settore_id AS id, nome, descrizione, attivo FROM settori WHERE attivo = true');
    res.json(rows);
});
router.get('/categorie', verifyToken, async (req, res) => {
    const [rows] = await pool.query('SELECT categoria_id AS id, nome, descrizione, attivo FROM categorie WHERE attivo = true');
    res.json(rows);
});
router.get('/marche', verifyToken, async (req, res) => {
    const [rows] = await pool.query('SELECT marca_id AS id, nome, descrizione, sito_web, attivo FROM marche WHERE attivo = true');
    res.json(rows);
});
